...
from oriskami import error, util
...
try:
    ...
    try:
        ...
        if (major, minor, patch) < (0, 8, 8):
            sys.stderr.write(
                'Warning: the Oriskami library requires that your Python '
                '"requests" library be newer than version 0.8.8, but your '
                '"requests" library is version %s. Oriskami will fall back to '
                'an alternate HTTP library so everything should work. We '
                'recommend upgrading your "requests" library. If you have any '
                'questions, please contact support@oriskami.com. (HINT: running '
...
def new_default_http_client(*args, **kwargs):
    if urlfetch:
        ...
        impl = Urllib2Client
        warnings.warn(
            "Warning: the Oriskami library is falling back to urllib2/urllib "
        ...
        try:
            try:
                ...
                raise TypeError(
                    ...
                    '"requests" library is not compatible with Oriskami\'s '
    ...
    def _handle_request_error(self, e):
        if isinstance(e, requests.exceptions.RequestException):
            msg = ("Unexpected error communicating with Oriskami.  "
                   ...
                   "support@oriskami.com.")
            err = "%s: %s" % (type(e).__name__, str(e))
        else:
            msg = ("Unexpected error communicating with Oriskami. "
                   "It looks like there's probably a configuration "
                   "issue locally.  If this problem persists, let us "
                   "know at support@oriskami.com.")
...
class UrlFetchClient(HTTPClient):
    ...
    def _handle_request_error(self, e, url):
        if isinstance(e, urlfetch.InvalidURLError):
            msg = ("The Oriskami library attempted to fetch an "
                   "invalid URL (%r). This is likely due to a bug "
                   "in the Oriskami Python bindings. Please let us know "
                   "at support@oriskami.com." % (url,))
        elif isinstance(e, urlfetch.DownloadError):
            msg = "There was a problem retrieving data from Oriskami."
        elif isinstance(e, urlfetch.ResponseTooLargeError):
            msg = ("There was a problem receiving all of your data from "
                   "Oriskami.  This is likely due to a bug in Oriskami. "
                   "Please let us know at support@oriskami.com.")
        else:
            msg = ("Unexpected error communicating with Oriskami. If this "
                   "problem persists, let us know at support@oriskami.com.")
...
class PycurlClient(HTTPClient):
    ...
    def _handle_request_error(self, e):
        if e[0] in [pycurl.E_COULDNT_CONNECT,
            ...
            msg = ("Could not connect to Oriskami.  Please check your "
                   "internet connection and try again.  If this problem "
                   "persists, you should check Oriskami's service status at "
                   "https://twitter.com/oriskamistatus, or let us know at "
                   "support@oriskami.com.")
        elif (e[0] in [pycurl.E_SSL_CACERT,
                       pycurl.E_SSL_PEER_CERTIFICATE]):
            msg = ("Could not verify Oriskami's SSL certificate.  Please make "
...
class Urllib2Client(HTTPClient):
    ...
    def _handle_request_error(self, e):
        msg = ("Unexpected error communicating with Oriskami. "
               "If this problem persists, let us know at support@oriskami.com.")
        msg = textwrap.fill(msg) + "\n\n(Network error: " + str(e) + ")"
        raise error.APIConnectionError(msg)
