...
import oriskami
from oriskami import error, http_client, version, util
...
def _api_encode(data):
    for key, value in data.items():
        ...
        if value is None:
            continue
        elif hasattr(value, 'oriskami_id'):
            yield (key, value.oriskami_id)
...
class APIRequestor(object):
    ...
    def __init__(self, key=None, client=None, api_base=None, api_version=None):
        self.api_base = api_base or oriskami.api_base
        self.api_key = key
        self.api_version = api_version or oriskami.api_version

        from oriskami import verify_ssl_certs as verify

        self._client = client or oriskami.default_http_client or \
    ...
    def request_headers(self, api_key, method):
        user_agent = 'Oriskami/v1 PythonBindings/%s' % (version.VERSION,)
        if oriskami.app_info:
            user_agent += " " + self.format_app_info(oriskami.app_info)
        ...
        for attr, func in [['lang_version', platform.python_version],
            ...
            ua[attr] = val
        if oriskami.app_info:
            ua['application'] = oriskami.app_info

        headers = {
            'X-Oriskami-Client-User-Agent': util.json.dumps(ua),
    ...
    def request_raw(self, method, url, params=None, supplied_headers=None):
        ...
        if self.api_key:
            ...
            from oriskami import api_key
            my_api_key = api_key

        if my_api_key is None:
            raise error.AuthenticationError(
                'No API key provided. (HINT: set your API key using '
                '"oriskami.api_key = <API-KEY>"). You can generate API keys '
                'from the Oriskami web interface.  See https://oriskami.com/api '
                'for details, or email support@oriskami.com if you have any '
                'questions.')
        ...
        if method == 'get' or method == 'delete':
            ...
            raise error.APIConnectionError(
                ...
                'Oriskami bindings.  Please contact support@oriskami.com for '
                'assistance.' % (method,))
        ...
        util.log_info('Request to Oriskami api', method=method, path=abs_url)
        util.log_debug(
        ...
        util.log_info(
            'Oriskami API response', path=abs_url, response_code=rcode)
