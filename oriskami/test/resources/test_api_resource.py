import oriskami
from oriskami.test.helper import (OriskamiApiTestCase, MyResource, MySingleton)


class APIResourceTests(OriskamiApiTestCase):
    ...
    def test_convert_to_oriskami_object(self):
        sample = {
            'foo': 'bar',
            'adict': {
                'object': 'charge',
        ...
        converted = oriskami.resource.convert_to_oriskami_object(sample, 'akey')

        # Types
        self.assertTrue(isinstance(converted, oriskami.resource.OriskamiObject))
    ...
    def test_convert_array_to_dict(self):
        out = oriskami.resource.convert_array_to_dict([{"foo": "bar"}])
        self.assertEqual({"0": {"foo": "bar"}}, out)
        self.assertEqual({"f": "b"},
                         oriskami.resource.convert_array_to_dict({"f": "b"}))


class SingletonAPIResourceTests(OriskamiApiTestCase):
