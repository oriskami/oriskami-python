...
class OriskamiTestCase(unittest2.TestCase):
    RESTORE_ATTRIBUTES = ('api_version', 'api_key', 'client_id')

    def setUp(self):
        super(OriskamiTestCase, self).setUp()

        self._oriskami_original_attributes = {}

        for attr in self.RESTORE_ATTRIBUTES:
            self._oriskami_original_attributes[attr] = getattr(oriskami, attr)

        api_base = os.environ.get('ORISKAMI_API_BASE')
        if api_base:
            oriskami.api_base = api_base
        oriskami.api_key = os.environ.get('ORISKAMI_TEST_TOKEN_PYTHON_1')

    def tearDown(self):
        super(OriskamiTestCase, self).tearDown()

        for attr in self.RESTORE_ATTRIBUTES:
            setattr(oriskami, attr, self._oriskami_original_attributes[attr])


class OriskamiUnitTestCase(OriskamiTestCase):
    ...
    def setUp(self):
        super(OriskamiUnitTestCase, self).setUp()

        self.request_patchers = {}
        self.request_mocks = {}
        for lib in self.REQUEST_LIBRARIES:
            patcher = patch("oriskami.http_client.%s" % (lib,))

            self.request_mocks[lib] = patcher.start()
            self.request_patchers[lib] = patcher

    def tearDown(self):
        super(OriskamiUnitTestCase, self).tearDown()

        for patcher in self.request_patchers.itervalues():
            patcher.stop()


class OriskamiAPIRequestorTestCase(OriskamiUnitTestCase):
    REQUESTOR_CLS = oriskami.api_requestor.APIRequestor

    def setUp(self):
        super(OriskamiAPIRequestorTestCase, self).setUp()

        self.http_client = Mock(oriskami.http_client.HTTPClient)
...
class OriskamiApiTestCase(OriskamiTestCase):
    REQUESTOR_CLS_NAME = 'oriskami.api_requestor.APIRequestor'

    def setUp(self):
        super(OriskamiApiTestCase, self).setUp()
    ...
    def tearDown(self):
        super(OriskamiApiTestCase, self).tearDown()
...
class OriskamiResourceTest(OriskamiApiTestCase):

    def setUp(self):
        super(OriskamiResourceTest, self).setUp()
        self.mock_response({})


class MyResource(oriskami.resource.APIResource):
    pass


class MySingleton(oriskami.resource.SingletonAPIResource):
    pass


class MyListable(oriskami.resource.ListableAPIResource):
    pass


class MyCreatable(oriskami.resource.CreatableAPIResource):
    pass


class MyUpdatable(oriskami.resource.UpdatableAPIResource):
    pass


class MyDeletable(oriskami.resource.DeletableAPIResource):
    pass


class MyComposite(oriskami.resource.ListableAPIResource,
                  oriskami.resource.CreatableAPIResource,
                  oriskami.resource.UpdatableAPIResource,
                  oriskami.resource.DeletableAPIResource):
    pass
