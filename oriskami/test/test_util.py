...
class UtilTests(OriskamiUnitTestCase):
    DUMMY_REQ_ID = 'req_qsxPhqHyLxcoaM'

    def test_test_apikey(self):
        with mock.patch('oriskami.api_key', 'sk_test_KOWobxXidxNlIx'):
            link = util.dashboard_link(self.DUMMY_REQ_ID)
            self.assertEqual(
                link,
                'https://dashboard.oriskami.com/test/logs/' + self.DUMMY_REQ_ID,
            )

    def test_live_apikey(self):
        with mock.patch('oriskami.api_key', 'sk_live_axwITqZSgTUXSN'):
            link = util.dashboard_link(self.DUMMY_REQ_ID)
            self.assertEqual(
                link,
                'https://dashboard.oriskami.com/live/logs/' + self.DUMMY_REQ_ID,
            )

    def test_no_apikey(self):
        with mock.patch('oriskami.api_key', None):
            link = util.dashboard_link(self.DUMMY_REQ_ID)
            self.assertEqual(
                link,
                'https://dashboard.oriskami.com/test/logs/' + self.DUMMY_REQ_ID,
    ...
    def test_old_apikey(self):
        with mock.patch('oriskami.api_key', 'axwITqZSgTUXSN'):
            link = util.dashboard_link(self.DUMMY_REQ_ID)
            self.assertEqual(
                link,
                'https://dashboard.oriskami.com/test/logs/' + self.DUMMY_REQ_ID,
        ...
        for case in test_cases:
            with self.subTest(env=case.env, flag=case.flag):
                try:
                    ...
                    self.patch_val('oriskami.log', case.flag)
                    self.patch_val('oriskami.util.ORISKAMI_LOG', case.env)
    ...
    def test_log_debug(self):
        # (ORISKAMI_LOG, oriskami.log): should_output?
        test_cases = [
        ...
        self.log_test_loop(
            ...
            logger_name='oriskami.util.logger.debug',
        )
    ...
    def test_log_info(self):
        ...
        self.log_test_loop(
            ...
            logger_name='oriskami.util.logger.info',
        )
