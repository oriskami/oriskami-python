...
from oriskami import api_requestor, error, util


def convert_to_oriskami_object(resp, api_key):
    types = {
            'router': Router,
    ...
    if isinstance(resp, list):
        return [convert_to_oriskami_object(i, api_key) for i in resp]
    elif isinstance(resp, dict) and not isinstance(resp, OriskamiObject):
        resp = resp.copy()
        klass_name = resp.get('object')
        if isinstance(klass_name, str):
            klass = types.get(klass_name, OriskamiObject)
        else:
            klass = OriskamiObject
...
class OriskamiObject(dict):
    def __init__(self, id=None, api_key=None, **params):
        super(OriskamiObject, self).__init__()
    ...
    def update(self, update_dict):
        ...
        return super(OriskamiObject, self).update(update_dict)

    def __setattr__(self, k, v):
        if k[0] == '_' or k in self.__dict__:
            return super(OriskamiObject, self).__setattr__(k, v)
    ...
    def __delattr__(self, k):
        if k[0] == '_' or k in self.__dict__:
            return super(OriskamiObject, self).__delattr__(k)
        else:
            del self[k]
    ...
    def __setitem__(self, k, v):
        ...
        super(OriskamiObject, self).__setitem__(k, v)
    ...
    def __getitem__(self, k):
        try:
            return super(OriskamiObject, self).__getitem__(k)
        except KeyError as err:
            if k in self._transient_values:
                raise KeyError(
                    "%r.  HINT: The %r attribute was set in the past."
                    "It was then wiped when refreshing the object with "
                    "the result returned by Oriskami's API, probably as a "
    ...
    def __delitem__(self, k):
        super(OriskamiObject, self).__delitem__(k)
    ...
    def refresh_from(self, values, api_key=None, partial=False):
        ...
        for k, v in values.items():
            super(OriskamiObject, self).__setitem__(
                k, convert_to_oriskami_object(v, api_key))
    ...
    def request(self, method, url, params=None, headers=None):
        ...
        return convert_to_oriskami_object(response, api_key)
    ...
    def __copy__(self):
        copied = OriskamiObject(self.get('id'), self.api_key)
        ...
        for k, v in self.items():
            ...
            super(OriskamiObject, copied).__setitem__(k, v)
    ...
    def __deepcopy__(self, memo):
        ...
        for k, v in self.items():
            # Call parent's __setitem__ to avoid checks that we've added in the
            # overridden version that can throw exceptions.
            super(OriskamiObject, copied).__setitem__(k, deepcopy(v, memo))

        return copied


class APIResource(OriskamiObject):
...
class ListObject(OriskamiObject):
...
class ListableAPIResource(APIResource):
    ...
    def list(cls, **params):
        ...
        oriskami_object = convert_to_oriskami_object(response, api_key)
        oriskami_object._retrieve_params = params
        return oriskami_object
