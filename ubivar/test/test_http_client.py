...
class ClientTestBase():
    ...
    def valid_url(self, path='/foo'):
        return 'https://api.ubivar.com%s' % (path,)
...
class RequestsVerify(object):
    ...
    def __eq__(self, other):
        return other and other.endswith('ubivar/data/ca-certificates.crt')


class RequestsClientTests(UbivarUnitTestCase, ClientTestBase):
...
class UrlFetchClientTests(UbivarUnitTestCase, ClientTestBase):
    request_client = ubivar.http_client.UrlFetchClient
...
class Urllib2ClientTests(UbivarUnitTestCase, ClientTestBase):
...
class PycurlClientTests(UbivarUnitTestCase, ClientTestBase):
    ...
    def setUp(self):
        ...
        self.sio_patcher = patch('ubivar.util.StringIO.StringIO')
    ...
    def mock_error(self, mock):
        ...
        ubivar.http_client.pycurl.error = FakeException
        mock.perform.side_effect = ubivar.http_client.pycurl.error
...
class APIEncodeTest(UbivarUnitTestCase):
    ...
    def test_encode_dict(self):
        ...
        values = [t for t in ubivar.api_requestor._api_encode(body)]
