import ubivar
from ubivar.test.helper import UbivarApiTestCase


class ListObjectTests(UbivarApiTestCase):

    def setUp(self):
        super(ListObjectTests, self).setUp()

        self.lo = ubivar.resource.ListObject.construct_from({
            'id': 'me',
            'url': '/my/path',
            'data': ['foo'],
        }, 'mykey')
    ...
    def assertResponse(self, res):
        self.assertTrue(isinstance(res[0], ubivar.Charge))
...
class AutoPagingTests(UbivarApiTestCase):
    ...
    def test_iter_one_page(self):
        lo = ubivar.resource.ListObject.construct_from(
    ...
    def test_class_method_two_pages(self):
        ...
        seen = [item['id'] for item in ubivar.Charge.auto_paging_iter(
            limit=25,
            foo='bar'
        )]
