...
import ubivar
from ubivar import util
from ubivar.test.helper import UbivarUnitTestCase, SAMPLE_INVOICE


class UbivarObjectTests(UbivarUnitTestCase):

    def test_initializes_with_parameters(self):
        obj = ubivar.resource.UbivarObject(
    ...
    def test_access(self):
        obj = ubivar.resource.UbivarObject('myid', 'mykey', myparam=5)
    ...
    def test_refresh_from(self):
        obj = ubivar.resource.UbivarObject.construct_from({
            'foo': 'bar',
            'trans': 'me',
        }, 'mykey')
        ...
        self.assertEqual(None, obj.ubivar_account)

        obj.refresh_from({
            'foo': 'baz',
            'johnny': 5,
        }, 'key2', ubivar_account='acct_foo')
        ...
        self.assertEqual('acct_foo', obj.ubivar_account)
    ...
    def test_passing_nested_refresh(self):
        ...
        self.assertEqual('acct_foo', nested.ubivar_account)
    ...
    def test_repr(self):
        ...
        self.assertTrue(u'<UbivarObject \u4e00boo\u1f00' in res)
    ...
    def test_deletion(self):
        obj = ubivar.resource.UbivarObject('id', 'key')
    ...
    def test_copy(self):
        nested = ubivar.resource.UbivarObject.construct_from({
            'value': 'bar',
        }, 'mykey')
        obj = ubivar.resource.UbivarObject.construct_from({
            'empty': '',
            'value': 'foo',
            'nested': nested,
        }, 'mykey', ubivar_account='myaccount')
        ...
        self.assertEqual('myaccount', copied.ubivar_account)
