...
class UtilTests(UbivarUnitTestCase):
    DUMMY_REQ_ID = 'req_qsxPhqHyLxcoaM'

    def test_test_apikey(self):
        with mock.patch('ubivar.api_key', 'sk_test_KOWobxXidxNlIx'):
            link = util.dashboard_link(self.DUMMY_REQ_ID)
            self.assertEqual(
                link,
                'https://dashboard.ubivar.com/test/logs/' + self.DUMMY_REQ_ID,
            )

    def test_live_apikey(self):
        with mock.patch('ubivar.api_key', 'sk_live_axwITqZSgTUXSN'):
            link = util.dashboard_link(self.DUMMY_REQ_ID)
            self.assertEqual(
                link,
                'https://dashboard.ubivar.com/live/logs/' + self.DUMMY_REQ_ID,
            )

    def test_no_apikey(self):
        with mock.patch('ubivar.api_key', None):
            link = util.dashboard_link(self.DUMMY_REQ_ID)
            self.assertEqual(
                link,
                'https://dashboard.ubivar.com/test/logs/' + self.DUMMY_REQ_ID,
    ...
    def test_old_apikey(self):
        with mock.patch('ubivar.api_key', 'axwITqZSgTUXSN'):
            link = util.dashboard_link(self.DUMMY_REQ_ID)
            self.assertEqual(
                link,
                'https://dashboard.ubivar.com/test/logs/' + self.DUMMY_REQ_ID,
        ...
        for case in test_cases:
            with self.subTest(env=case.env, flag=case.flag):
                try:
                    ...
                    self.patch_val('ubivar.log', case.flag)
                    self.patch_val('ubivar.util.STRIPE_LOG', case.env)
    ...
    def test_log_debug(self):
        # (STRIPE_LOG, ubivar.log): should_output?
        test_cases = [
        ...
        self.log_test_loop(
            ...
            logger_name='ubivar.util.logger.debug',
        )
    ...
    def test_log_info(self):
        ...
        self.log_test_loop(
            ...
            logger_name='ubivar.util.logger.info',
        )
