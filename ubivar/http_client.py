...
try:
    ...
    try:
        ...
        if (major, minor, patch) < (0, 8, 8):
            sys.stderr.write(
                'Warning: the Ubivar library requires that your Python '
                '"requests" library be newer than version 0.8.8, but your '
                '"requests" library is version %s. Ubivar will fall back to '
                'an alternate HTTP library so everything should work. We '
                'recommend upgrading your "requests" library. If you have any '
                'questions, please contact support@ubivar.com. (HINT: running '
...
def new_default_http_client(*args, **kwargs):
    if urlfetch:
        ...
        impl = Urllib2Client
        warnings.warn(
            "Warning: the Ubivar library is falling back to urllib2/urllib "
        ...
        try:
            try:
                ...
                raise TypeError(
                    ...
                    '"requests" library is not compatible with Ubivar\'s '
...
class UrlFetchClient(HTTPClient):
    ...
    def _handle_request_error(self, e, url):
        if isinstance(e, urlfetch.InvalidURLError):
            msg = ("The Ubivar library attempted to fetch an "
                   "invalid URL (%r). This is likely due to a bug "
                   "in the Ubivar Python bindings. Please let us know "
                   "at support@ubivar.com." % (url,))
        elif isinstance(e, urlfetch.DownloadError):
            msg = "There was a problem retrieving data from Ubivar."
        elif isinstance(e, urlfetch.ResponseTooLargeError):
            msg = ("There was a problem receiving all of your data from "
                   "Ubivar.  This is likely due to a bug in Ubivar. "
                   "Please let us know at support@ubivar.com.")
        else:
            msg = ("Unexpected error communicating with Ubivar. If this "
                   "problem persists, let us know at support@ubivar.com.")
...
class PycurlClient(HTTPClient):
    ...
    def _handle_request_error(self, e):
        if e[0] in [pycurl.E_COULDNT_CONNECT,
            ...
            msg = ("Could not connect to Ubivar.  Please check your "
                   "internet connection and try again.  If this problem "
                   "persists, you should check Ubivar's service status at "
                   "https://twitter.com/ubivarstatus, or let us know at "
                   "support@ubivar.com.")
        elif (e[0] in [pycurl.E_SSL_CACERT,
                       pycurl.E_SSL_PEER_CERTIFICATE]):
            msg = ("Could not verify Ubivar's SSL certificate.  Please make "
...
class Urllib2Client(HTTPClient):
    ...
    def _handle_request_error(self, e):
        msg = ("Unexpected error communicating with Ubivar. "
               "If this problem persists, let us know at support@ubivar.com.")
        msg = textwrap.fill(msg) + "\n\n(Network error: " + str(e) + ")"
        raise error.APIConnectionError(msg)
